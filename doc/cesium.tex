...
The \code{if.. else} statement has identical syntax to the \code{if} expression, the distinction being
that in the latter case, the final statement in each branch is an expression rather than a statement.
...
the same type) is called an \code{if} expression. It does return a value, whose type is the same
as that of the two expressions. 
...
In this example, the expression will have the value \code{4} because the first branch is taken
and the final expression in that branch has the value \code{4}.
